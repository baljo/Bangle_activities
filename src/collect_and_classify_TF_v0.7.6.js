...
");");
    ...
    if (storageFile) {
      let now = new Date();
      if ((now - currentActivityStart) >= 20000) {
...
function performInference() {
  ...
  if (activity !== detectedActivity && (new Date() - activityTracking[detectedActivity].startTime) >= 20000) {
...
function trackActivity(detectedActivity) {
  ...
  if ((new Date() - activityTracking[detectedActivity].startTime) >= 20000) {

...
function performInference() {
  ...
  try {
    ...
    let activityIndex = argMax(output);
    let detectedActivity = getActivityLabel(activityIndex);

    // Show the detected activity on the watch screen
    console.log(detectedActivity);
    showStatus("Detected: " + detectedActivity);
  } catch (e) {
    // Skip logging error messages to avoid float-related errors
  }
}

// Utility to find index of maximum value in an array (used for prediction output)
function argMax(arr) {
